...
function makeAuthorizerContext(
  payload: Record<string, string>,
): APIGatewayEventLambdaAuthorizerContext<Record<string, string>> {
  // The AWS type intersection (Record<string, string> & { integrationLatency: number })
  // is not constructable from a literal. We use Object.assign to satisfy the runtime shape.
  return Object.assign({ principalId: "user", integrationLatency: 0 }, payload);
}
...
function makeV1EventWithCustomAuthorizer(
  ...
  return {
    ...
    requestContext: {
      accountId: "123",
      apiId: "abc",
      authorizer: makeAuthorizerContext(authPayload),
      httpMethod: "GET",
      identity: {
        accessKey: null,
        accountId: null,
...
describe("asLambdaAuthorizedHttpV2Handler", () => {
  test("passes status code through", async () => {
    const handler = asLambdaAuthorizedHttpV2Handler(async () => new Response("", { status: 200 }));
    const result = await handler(makeV2EventWithLambdaAuthorizer({ userId: "u1" }), makeContext());
...
describe("asCustomAuthorizedHttpV1Handler", () => {
  test("passes status code through", async () => {
    const handler = asCustomAuthorizedHttpV1Handler(async () => new Response("", { status: 200 }));
    const result = await handler(makeV1EventWithCustomAuthorizer({ userId: "u2" }), makeContext());
    expect(result.statusCode).toBe(200);
  });
