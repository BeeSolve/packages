...
export class AuthWithPasskeysStack extends Stack {
  constructor(scope: App, id: string, props: StackProps) {
    super(scope, id, props);

    const emailAddress = process.env.SAMPLES_EMAIL_ADDRESS;
    const frontendUri = process.env.SAMPLES_AUTH_WITH_PASSKEYS_FRONTEND_URI;

    if (emailAddress == null || frontendUri == null) {
      throw new Error(
        "SAMPLES_EMAIL_ADDRESS and SAMPLES_AUTH_WITH_PASSKEYS_FRONTEND_URI must be set (see mise.toml)",
      );
    }
    ...
    new Rule(this, "AuthEventsRule", {
      eventPattern: {
        ...
        detailType: ["EmailCodeAuth", "UnsuccessfulAuth", "PasskeyRegistered", "PasskeyAuthUsed"],
      },
