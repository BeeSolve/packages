...
describe("Backfill", () => {
  describe("startRun", () => {
    it("sends a single TransactWriteCommand with a guarded config Update and a guarded history Put", async () => {
      ...
      // Guards on the nested status: only terminal statuses (finished/failed)
      // or a missing entry/attribute may start a run.
      expect(condition).toContain("attribute_not_exists(#domains)");
