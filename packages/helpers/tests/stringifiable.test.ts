...
import { decodeFromStringifiable, encodeToStringifiable } from "../index";
...
describe("encodeToStringifiable / decodeFromStringifiable", () => {
  ...
  test("round-trips an array", () => {
    expect(roundTrip([1, "two", null, undefined])).toEqual([1, "two", null, undefined]);
  });
  ...
  test("throws when encoding a function", () => {
    expect(() => encodeToStringifiable(() => {})).toThrow("Cannot encode function");
  });

  test("throws when encoding a symbol", () => {
    expect(() => encodeToStringifiable(Symbol("x"))).toThrow("Cannot encode symbol");
  });
  ...
  test("throws when decoding a non-plain-object", () => {
    ...
    expect(() => decodeFromStringifiable(null)).toThrow("Only plain objects can be decoded.");
    expect(() => decodeFromStringifiable([1, 2, 3])).toThrow("Only plain objects can be decoded.");
  });
});
