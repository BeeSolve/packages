...
- [x] Create `packages/dmarc-consumer/src/runDnsRefresh.ts` (mirror `src/runBackfill.ts`):
  - `runDnsRefresh({ reports, domains, jobs, domain, runId }): Promise<void>` — derive observed DKIM selectors by scanning the domain's stored reports (via `Reports`, same source `runBackfill` uses for IPs; collect unique `authResults.dkim[].selector`), call `resolveDomainDns({ domain, dkimSelectors })`, `domains.putDns({ domain, dns })`, then mark the run `finished` (with `counts.selectorsChecked`) or `failed` via the generalized tracker for the `dnsRefresh` kind.
- [x] In `packages/dmarc-consumer/src/tasks.ts`, add `refreshDomainDns` to the existing `createSqsHandlers` `functions` map (alongside `backfillDomain`). Instantiated the shared `Domains` + `dnsRefresh`-kind `JobRuns`; added `REVERSE_INDEX_NAME` to the tasks env.
- [x] Create `packages/dmarc-consumer/src/dnsCron.ts` — EventBridge handler: `domains.list()`, filter with `isDnsStale`, enqueue `tasks.refreshDomainDns` through the **same guarded `startRun`** (shared `isAlreadyRunning` extracted to `jobRuns.ts`). Pure `enqueueStaleDomains` for testing; TTL from `DNS_CACHE_TTL_MS` (default 24h).
- [x] In `packages/dmarc-consumer/cdk.ts`: added a daily `Rule` (`Schedule.rate(Duration.days(1))`) targeting a new `Nodejs24Function` for `dnsCron` with table read + `this.backfill.grantAccess` + `TABLE_NAME`/`REVERSE_INDEX_NAME`. Added `REVERSE_INDEX_NAME` to the tasks handler env; `build.ts` builds the `dnsCron` entry.
- [x] Include tests: `packages/dmarc-consumer/tests/dnsCron.test.ts` (only stale/missing enqueue, already-running skip, all-fresh no-op), `tests/runDnsRefresh.test.ts` (selector scan + resolve + write + complete; failure path), extended `tests/cdk.test.ts` (daily rule + cron fn).
...
- [x] `packages/dmarc-dashboard/src/hooks.server.ts` + `src/app.d.ts`: already wired in Task 4 — `adminSdk` (real `AdminSdk`) and `domains` are in `event.locals.services` (single-instantiation); no `dnsCache` exists. No further change needed.
- [x] In `packages/dmarc-dashboard/src/routes/domains/[domain]/+page.server.ts` `load`: reads cached DNS via `domains.getByDomain`, computes `advisory = buildAdvisory({ dns, aggregate })`, and surfaces `dnsRefreshStatus` + `ipBackfillStatus` (per-domain `canRun`/`lastRun`, fetched in parallel). Returns `dns`, `advisory`, `dnsRefreshStatus`, `ipBackfillStatus`. Access check enforced; SSR is read-only (no inline resolution).
- [x] **Single default form action with a hidden `intent` field** (no named actions): validates `intent` via `v.picklist(["refresh-dns", "refresh-ips"])`; `refresh-dns` → `adminSdk.startDnsRefresh`, `refresh-ips` → `adminSdk.startIpBackfill`. Same admin/domain access check as `load`; `already-running` → 409-style `fail` carrying the `intent`.
